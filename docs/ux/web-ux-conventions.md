...
- **List-first, thumb-zone CTAs:** primary settle / create actions live in bottom bars and FABs; prefer status cards and lists over chart dashboards.
- **Optional transactional notes:** money notes (Pay, New Order commercial cash/security, loan repayment/disbursement/adjustment, return/cancel) are optional free text ≤20 — never block settle solely because the note is empty.
- **Immediate search:** Home/global, Resources, Customers, and Transactions filters/typeahead activate on the first non-empty trimmed character; do not show a min-length helper under search fields.
...
- Search filters and typeahead activate on the **first** typed character after trim (`kMinMeaningfulTextLength = 1`). Do **not** show a “Type at least N characters” helper under search fields (focused or otherwise).
...
- Free-text identity fields (customer name, item name, category, instance name, SELF nickname, and optional catalog notes when non-empty) must be non-empty when required — same shared floor as search (`kMinMeaningfulTextLength = 1` after trim). **Order notes** body (order detail append-only) still requires ≥3 characters. Search (Home/global = all entities; Resources = resources; Customers = customers + order nicknames; Transactions list filter) runs from the first non-empty trimmed character.
...
- Returns always require an explicit confirm dialog. Show the computed line total (read-only), an editable **final amount to collect** (0…total; remainder shown as discount), deposit preview against that final amount (from the order deposit), and an optional note (≤20 chars free text).
- **Mark complete** (job lines) uses a simple confirm; stock is not restored.
- **Cancel order** is available on open orders with no rent/job lines already settled, as the last control in the order-detail bottom action bar (danger-styled outline; not the primary filled CTA). Confirmation settles order deposit as amount kept + amount returned (both default 0; sum ≤ order deposit remaining) plus optional note (≤20 chars free text), then cancels the order and restores stock.
