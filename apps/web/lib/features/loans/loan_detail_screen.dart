...
class _LoanDetailScreenState extends ConsumerState<LoanDetailScreen> {
  ...
  Future<void> _editSetup(
    ...
    final AppLocalizations l10n = context.l10n;
    MoneyLoanDirection direction = loan.direction;
    MoneyRatePeriod ratePeriod = loan.ratePeriod;
    MoneyCapitalizationPolicy capPolicy = loan.capitalizationPolicy;
    MoneyCapitalizationCycle capCycle = loan.capitalizationCycle;
    MoneyPrepaymentAllocation prepaymentAllocation = loan.prepaymentAllocation;
    ...
    final TextEditingController principalCtrl = TextEditingController(
      text: paiseToRupeesField(loan.principalPaise),
    );
    final TextEditingController rateCtrl = TextEditingController(
      text: loan.rateBps % 100 == 0
          ? '${loan.rateBps ~/ 100}'
        ...
        return StatefulBuilder(
          builder: (BuildContext context, StateSetter setDialog) {
            ...
            return AlertDialog(
              ...
              content: SizedBox(
                width: mq.size.width > 520 ? 460 : mq.size.width * 0.92,
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      SegmentedButton<MoneyLoanDirection>(
                        ...
                        selected: <MoneyLoanDirection>{direction},
                        onSelectionChanged: (Set<MoneyLoanDirection> s) {
                          setDialog(() => direction = s.first);
                        },
                      ),
                      const SizedBox(height: 16),
                      TextField(
                              ...
                              onChanged: (MoneyRatePeriod? v) {
                                if (v != null) {
                                  setDialog(() {
                                    ratePeriod = v;
                                    if (capPolicy ==
                                        MoneyCapitalizationPolicy
                                            .onScheduledCycle) {
                                      capCycle =
                                          MoneyCapitalizationCycle
                                              .fromRatePeriod(v);
                                    }
                                  });
                                }
                      ...
                      DropdownButtonFormField<MoneyCapitalizationPolicy>(
                        ...
                        items: <DropdownMenuItem<MoneyCapitalizationPolicy>>[
                          ...
                          DropdownMenuItem<MoneyCapitalizationPolicy>(
                            value: MoneyCapitalizationPolicy
                      ...
                      if (capPolicy ==
                          MoneyCapitalizationPolicy.onScheduledCycle) ...<Widget>[
                        const SizedBox(height: 12),
                      ...
                      SegmentedButton<MoneyPrepaymentAllocation>(
                        ...
                        selected: <MoneyPrepaymentAllocation>{
                          prepaymentAllocation,
                        },
                        onSelectionChanged:
                            (Set<MoneyPrepaymentAllocation> s) {
                          setDialog(() => prepaymentAllocation = s.first);
                        },
                      ),
                      const SizedBox(height: 8),
                      Text(
              ...
              actions: <Widget>[
                ...
                FilledButton(
                  onPressed: () {
                    final int principal =
                        parseRupeesToPaise(principalCtrl.text);
                    if (principal <= 0) {
                    ...
                    if (note.isNotEmpty && !meetsMinMeaningfulText(note)) {
                      ScaffoldMessenger.of(this.context).showSnackBar(
                        SnackBar(
                          content: Text(
    ...
    final int principalPaise = parseRupeesToPaise(principalCtrl.text);
    final double? ratePct = double.tryParse(rateCtrl.text.trim());
    final String noteText = noteCtrl.text.trim();
    ...
    if (principalPaise <= 0 || ratePct == null || ratePct < 0) {
      return;
    }
    try {
      await ref.read(repositoryProvider).updateMoneyLoan(
            ...
            capitalizationPolicy: capPolicy,
            capitalizationCycle: capCycle,
            rateBps: (ratePct * 100).round(),
